...
/*
 * 2D stencil code parallelized by multiple threads with MPI_THREAD_MULTIPLE using
 * MPI_COMM_WORLD.
 *
...
#pragma omp barrier
            ...
            if (my_south != MPI_PROC_NULL)
                MPI_Irecv(south_in, xrange, MPI_DOUBLE, my_south, Th_dim_x, MPI_COMM_WORLD, &reqs[0]);
            else
                reqs[0] = MPI_REQUEST_NULL;

            if (my_north != MPI_PROC_NULL)
                MPI_Irecv(north_in, xrange, MPI_DOUBLE, my_north, Th_dim_x, MPI_COMM_WORLD, &reqs[1]);
            else 
                reqs[1] = MPI_REQUEST_NULL;
            
            if (my_west != MPI_PROC_NULL)
                MPI_Irecv(west_in, yrange, MPI_DOUBLE, my_west, Th_dim_y, MPI_COMM_WORLD, &reqs[2]);
            else
                reqs[2] = MPI_REQUEST_NULL;
            
            if (my_east != MPI_PROC_NULL)
                MPI_Irecv(east_in , yrange, MPI_DOUBLE, my_east, Th_dim_y, MPI_COMM_WORLD, &reqs[3]);
            else    
                reqs[3] = MPI_REQUEST_NULL;
            
            if (my_south != MPI_PROC_NULL)
                MPI_Isend(south_out, xrange, MPI_DOUBLE, my_south, Th_dim_x, MPI_COMM_WORLD, &reqs[4]);
            else
                reqs[4] = MPI_REQUEST_NULL;
            
            if (my_north != MPI_PROC_NULL)
                MPI_Isend(north_out, xrange, MPI_DOUBLE, my_north, Th_dim_x, MPI_COMM_WORLD, &reqs[5]);
            else
                reqs[5] = MPI_REQUEST_NULL;
            
            if (my_west != MPI_PROC_NULL)
                MPI_Isend(west_out, yrange, MPI_DOUBLE, my_west, Th_dim_y, MPI_COMM_WORLD, &reqs[6]);
            else
                reqs[6] = MPI_REQUEST_NULL;
            
            if (my_east != MPI_PROC_NULL)
                MPI_Isend(east_out, yrange, MPI_DOUBLE, my_east, Th_dim_y, MPI_COMM_WORLD, &reqs[7]);
            else
